  ...
  while (1) {
    if (!odometryBuf.empty() && !pointCloudBuf.empty()) {
      ...
      if (!pointCloudBuf.empty() &&
        ...
        ROS_WARN(
            "time stamp unaligned error and pointcloud discarded, pls check "
            "your data --> laser mapping node");
        pointCloudBuf.pop();
        mutex_lock.unlock();
        continue;
      }
      ...
      if (!odometryBuf.empty() &&
        ...
        odometryBuf.pop();
        ROS_INFO(
            "time stamp unaligned with path final, pls check your data --> "
            "laser mapping node");
        mutex_lock.unlock();
        continue;
      }
      ...
      current_pose.rotate(
          Eigen::Quaterniond(odometryBuf.front()->pose.pose.orientation.w,
      ...
      current_pose.pretranslate(
          Eigen::Vector3d(odometryBuf.front()->pose.pose.position.x,
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>(
      "/velodyne_points_filtered", 100, velodyneHandler);
  ros::Subscriber subOdometry =
      nh.subscribe<nav_msgs::Odometry>("/odom", 100, odomCallback);
