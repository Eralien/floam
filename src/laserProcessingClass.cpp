...
void LaserProcessingClass::featureExtractionFromSector(
  ...
  //         for(int k=1;k<=5;k++){
  //             double diffX = pc_in->points[ind + k].x - pc_in->points[ind + k
  //             - 1].x; double diffY = pc_in->points[ind + k].y -
  //             pc_in->points[ind + k - 1].y; double diffZ = pc_in->points[ind
  //             + k].z - pc_in->points[ind + k - 1].z; if (diffX * diffX +
  //             diffY * diffY + diffZ * diffZ > 0.05){
  //                 break;
  //             }
  //             picked_points.push_back(ind+k);
  //         }
